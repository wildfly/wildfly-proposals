...
the feature-pack config stability level is used.
* A user can specify a package stability level, by means of the `package-stability-level` Galleon option. 
This stability level is used to constrain the stability level of all provisioned packages. NB: This option can't be set when `stability-level` is set. 
If the specified level is lower than a feature-pack package stability, 
the feature-pack package stability level is used.
