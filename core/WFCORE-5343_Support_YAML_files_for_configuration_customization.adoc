...
If we support more operations (like delete) then we will update the current list of operation to reflect that. This means that if an attribute is updated in a YAML file, then the resource is deleted in a seconfd one then the first operation won't be executed.
...
Note that the `!remove` operation is only for resources and not for attribute. If you want to 'remove' an attribute the proper way is to `!undefine` it.
...
That means that at least those entries would be *ignored*:
 ...
 - deployment: to add *managed* deployments to the server as this require more that just some configuration.
...
While the YAML configuration files aim to be able to be resuse accross versions, it depends mostly on the evolution of the configuration meta model which is out of the scope of this initial proposition.
If an attribute or a resource changes accross version are such that the YAML extension can't find it (like a resource moving in the tree or an attribute being removed or renamed) then the YAML configuration will become stale and will have to be updated accordingly.
*So the feature don't aim to be retro-compatible*. We might provide a tool to migrate the YAML configuration files in the future.
...
For bootable jar the idea is to be able to use the YAML files using the same command line argument like this: 

`java -Dyaml=test.yml -jar target/myapp-bootable.jar`

Thus it could be defined on OpenShift using the `JAVA_ARGS` environment variable like this:

`JAVA_ARGS="-y simple.yaml:simple2.yaml"``
