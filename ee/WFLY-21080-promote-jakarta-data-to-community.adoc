 ...
 * When our OOTB configuration files are provisioned using a feature pack that provides Jakarta EE 11 support, the `jakarta-data` subsystem will be present in the  `standalone.xml`, `standalone-full.xml`, `standalone-ha.xml` and `standalone-full-ha.xml`, `standalone-microprofile.xml` and `standalone-microprofile.xml` configuration files, and also in any `domain.xml` profiles whose intended feature set aligns with one of those standalone variant..
...
* HAL support at least equivalent to other https://docs.wildfly.org/38/Admin_Guide.html#Simple_configuration_subsystems[simple configuration subsystems], i.e. ones that expose no attributes or child resources.
