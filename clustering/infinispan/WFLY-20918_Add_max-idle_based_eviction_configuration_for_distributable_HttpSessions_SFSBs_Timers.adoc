...
* https://issues.redhat.com/browse/WFLY-20839[WFLY-20839] Remove max-active-timers from default configuration (not in the scope of the RFE)
...
WildFly now supports idle time-based eviction (using the `idle-thresold` attributes) for distributable HttpSessions, Stateful JEB Beans, and JEB Timers.
