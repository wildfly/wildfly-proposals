...
use a different HTTP authentication mechanism configuration which will use the `BEARER` HTTP 
...
resource will be used instead of the individual attributes on the management interface.  This attribute
will default to `false` and will need to be explicitly configured.

The existing attributes, `ssl-context`, `sasl-authentication-factory`, and 
`http-authentication-factory` will not be mutually exclusive / `altertives` to the new attribute,
instead individual capabilities can override the capabilities from the security policy.
...
This RFE only adds a new capability that can be used to provide an alternative approach to securing
HTTP access to the management interfaces, existing configurations using Elytron capabiltiies 
directly to secure the management interfaces will continue to work as they always did.  There are no
current plans to disable securing the management interfaces as they have been secured previously.

==== Default Configuration

The new subsystem will be present in the default configuration with a FORM configuration to make
use of the existing `ManagementDomain` for authentication.

The management interfaces however will not use FORM authentication out of the box.  The reason is 
that when activating FORM authentication TLS should also be enabled for the management interfaces
at the same time, this way we can document the steps and considerations to take when activating
this feature.

=== Nice-to-Have Requirements

The CLI will have a new security command adding to activate FORM authentication for the HTTP management
interface, this will largely be the case of setting `use-core-management-security-policy` to `true`
and clearing the other attributes.  

=== Non-Requirements

The integration within the admin console such as how to handle the need for authentication and how to
activate FORM authentication will be handled under a separate RFE so are not a requirement of this RFE.

The architecture chosen for this RFE will form the basis for future development which will not be 
covered by this RFE.
...
DIGEST based, this change updates the configuration to use a security policy within the 
...
Testing will need to be performed in a couple of different locations.

 * Within WildFly Elytron any new components will have their own set of unit tests.
 * Tests will be added to either WildFly Core or WildFly to test the new authentication flows once activated.
 * The WildFly testsuite is not a good location to test the appearance of the FORM so that may be delegated
to the admin console testsuite.
 * The use of MFA will require a custom security realm, a test will be developed in the WildFly testsuite
for this scenario.  This test will also be the base example for users looking to use this feature.
