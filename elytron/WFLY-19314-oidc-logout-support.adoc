...
* `post-logout-path`: This option designates the path component of a URL
the user has registered as the `post_logout_redirect_uri` in the OP.
Elytron uses it to generate an appropriate URL for a redirect call.
There is no default value. If left unset it is not used.
- The text can not be an empty string.
- It is expected that the text start with a forward slash (e.g. "/").
- It can be a sequence of path segments (e.g. "/call/back/home")
