...
Mixed domains must continue to work, of course, so a transformer must be added to the system that, for starting older servers on versions of Wildfly discards, the `graceful-startup` attribute when the value is set to the default, or rejects it otherwise, preventing the older server from starting. This will require the administrator to make an explicit decision about non-graceful startups in a mixed domain environment. The requirement that that Domain Controller be at least the same version, if not greater, than the managed Host Controllers remains unchanged.
