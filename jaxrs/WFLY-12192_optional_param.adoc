...
Please note that `@PathParam` is an exception and that using `Optional` with `@PathParam` doesn't allow us to omit that parameter from the path. The reason is that the path param must be exist in path, and when we don't have it in path, actually we have a different path and it will either be served by a method mapped to that different path, or that path doesn't exist.
