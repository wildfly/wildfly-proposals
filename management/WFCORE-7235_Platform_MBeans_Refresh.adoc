...
* Evolve the `/core-service=platform-mbean/type=*` WildFly resources with new standard attributes and operations defined in JDK11
* Expose through the same WildFly management resources the attributes and operations defined in the `com.sun.management` MXBeans.
...
The `/core-service=platform-mbean` resource has been evolved to expose new platform MXBeans, attributes and operations.
