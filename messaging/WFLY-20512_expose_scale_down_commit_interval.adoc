...
Those attributes shouldn't have any default value so that the current behaviour is maintained aka trandferring all the messages from the stopping node to the live node.
