...
* log-requests-responses: enabling the tracing of requests and responses going to Gemini.
* max-output-token: the number of token returned by the Gemini chat model.
...
It should also support Github.
It should expose a simple `github-chat-model` resource with the following attributes:

* api-key: the API key to authenticate to a Github chat model.
* connect-timeout: the timeout for the Github chat model.
* custom-headers: custom headers.
* endpoint: the endpoint to connect to a Github chat model.
* frequency-penalty: the frequency penalty of the Github chat model.
* log-requests-responses: enabling the tracing of requests and responses going to Github.
* max-retries: the maximum number of retries.
* max-token: the number of token returned by the Github chat model.
* model-name: the name of the model served by Github.
* presence-penalty: the presence penalty of the Github chat model.
* response-format: the response format.
* seed: the seed of the Github chat model.
* service-version: the service version. Allowed values: 2024-05-01-preview.
* streaming: enables streaming.
* temperature: the temperature of the Github chat model.
* top-p: the top P of the Github chat model.
* user-agent-suffix: the user agent suffix.

----
/subsystem=ai/github-chat-model=mychat:add(api-key="${env.GITHUB_API_KEY}", endpoint="https://api.github.com/copilot_internal/v2/token", model-name="copilot-chat")
----
