...
a logging profile, the loggers may end up getting created on the wrong `LogContext`. For example if `a.war` and `b.war`
...
used it would be that profiles log context.

https://issues.jboss.org/browse/LOGMR-264[LOGMR-264] will introduce a way to route log messages based on the log context.
This would allow messages from modules to be routed to deployment specific log contexts. It would also introduce a
change in behavior which is the reason for an RFE. A new attribute should be introduce to disable the new routing and
...
* https://issues.jboss.org/browse/LOGMR-264[LOGMR-264]
...
* The attribute will be a simple boolean attribute which determines if the new or old behavior will be used.
...
The logging subsystem documentation will be updated to show the new attribute. Detailed examples will be given on how
logging is routed when the new behavior is enabled.
