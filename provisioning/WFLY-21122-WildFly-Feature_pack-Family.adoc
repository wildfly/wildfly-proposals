...
* `microprofile-7+`: An implementation of the MicroProfile platform spec with min version being 7. 
A feature-pack implementing this criterion provides the APIs, spec impls and Galleon resources 
...
criteria to inherit can be operated (see `Implementation notes` fo details).
...
* WildFly feature-pack: `microprofile-7+`, `microprofile-7.1` + the following ones inherited from WildFly EE : `jakarta-ee-deployment`, `jakarta-ee-10+`
* WildFly Preview feature-pack: `jakarta-ee-deployment`, `jakarta-ee-10+`, `jakarta-ee-11`, `microprofile-7+`, `microprofile-7.1`
...
NOTE: That is outside of this proposal but WildFly and WildFly Preview feature-packs, 
in order to be valid alternatives, must expose a common set of Galleon `features`, `packages` and `layers`.
...
==== Potential impact for WildFly Glow

Today WildFly Glow CLI and its integration in the WildFly Maven Plugin offer the `wildfly-preview` option to contextualize the feature-packs discovery to use WildFly Preview feature-packs. This option should be deprecated
...
* A new feature: `WildFly family and criteria` with a link to this proposal.