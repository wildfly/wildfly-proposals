...
* Use a `mp.openapi.extensions.relative` property to configure whether auto-generated Server records should be absolute or relative.
** When relative, the generated record will contain the context path of the deployment
** When absolute, generated records will include all hosts/ports at which the given deployment is accessible.
